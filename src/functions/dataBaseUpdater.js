...
class DataBaseUpdater {
  async init (args) {
    ...
    try {
      ...
      code = 2
      try {
        const { data } = await this.scraper(date)
        response = cleanerForScraper(data)
      } catch (error) {
        this.mailer(
          null,
          true,
  ...
  mailer (code, error, date, message = null) {
    ...
    if(error) {
      ...
      if(code === 1) text += 'It was updated from the API.'
      else text += 'It was updated from the situational room.'
    }
    ...
    try {
      ...
      const data = await page.evaluate(
        async ({ date, selectors }) => {
          ...
          let departments = fDepartments.concat(mDepartments.slice(1, 5))
          ...
          const totalRecovered = covidCases
            .slice(0, 20)
            .concat(covidCases.slice(100).slice(0, 6))
